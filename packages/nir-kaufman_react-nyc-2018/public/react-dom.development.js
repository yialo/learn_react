...
(function (global, factory) {
        ...
        switch (fiber.tag) {
          ...
          case SimpleMemoComponent: {
            var renderingComponentName =
              (workInProgress && getComponentName(workInProgress.type)) || 'Unknown'; // Dedupe by the rendering component because it's the one that needs to be fixed.
  ...
  function isSimpleFunctionComponent(type) {
    return (
      typeof type === 'function' && !shouldConstruct$1(type) && type.defaultProps === undefined
    );
  }
      ...
      if (fiber === undefined) {
        if (typeof component.render === 'function') {
        ...
        } else {
          {
            {
              throw Error(
                'Argument appears to not be a ReactComponent. Keys: ' + Object.keys(component),
              );
            }
          }
        }
